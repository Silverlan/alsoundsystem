...
#ifndef __ALSOUND_STEAM_AUDIO_PROPERTIES_HPP__
...
namespace al
{
	...
	{
		enum class SpatializerInterpolation : uint32_t
		{
			Nearest = 0u,
			Bilinear
		};
		enum class SpatializerOcclusionMode : uint32_t
		{
			Off = 0u,
			NoTransmission,
			FrequencyIndependentTransmission,
			FrequencyDependentTransmission
		};
		enum class OcclusionMethod : uint32_t
		{
			Raycast = 0u,
			Partial
		};
		enum class IndirectType : uint32_t
		{
			RealTime = 0u,
			Baked
		};
