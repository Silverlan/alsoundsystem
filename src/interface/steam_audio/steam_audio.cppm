...
export namespace ipl {
	...
	class Context;
	class Environment;
	class DLLALSYS Scene : public std::enable_shared_from_this<Scene> {
	  public:
		enum class LoadStage : uint32_t { LoadFromDisk = 0, FinalizingScene, GeneratingProbes, BakingReverb, BakingPropagation };
		...
		enum class DSPEffect { Spatializer, Reverb };
	  ...
	  private:
		struct DLLALSYS ProbeBox {
			ProbeBox() = default;
			ProbeBox(const Vector3 &min, const Vector3 &max, float spacing, float heightAboveFloor, bool centroid = false) : min(min), max(max), spacing(spacing), heightAboveFloor(heightAboveFloor), centroid(centroid) {}
			ProbeBox(const Vector3 &origin, float radius) : ProbeBox(origin, origin, radius, 0.f, true) {}
			Vector3 min = {};
			Vector3 max = {};
			float spacing = 512.f;
	...
	class DLLALSYS Context : public std::enable_shared_from_this<Context> {
	  public:
