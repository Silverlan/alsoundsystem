...
ipl::StaticMesh::StaticMesh(Scene &scene,const std::vector<IPLVector3> &verts,const std::vector<IPLTriangle> &tris,const std::vector<IPLint32> &materialIndices)
	: m_scene(scene.shared_from_this()),m_vertices(verts),m_triangles(tris),m_materialIndices(materialIndices)
{}
...
bool ipl::StaticMesh::Spawn()
{
	...
	m_iplMesh = std::shared_ptr<void>(iplMesh,[](IPLhandle iplMesh) {
		iplDestroyStaticMesh(&iplMesh);
	});
