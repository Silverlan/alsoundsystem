...
ipl::Scene::Scene(Context &context)
	: std::enable_shared_from_this<Scene>(),m_context(context.shared_from_this())
{
...
{
	auto it = std::find_if(m_propagationSoundSources.begin(),m_propagationSoundSources.end(),[&name](const PropagationSoundSource &src) {
		return ustring::compare(name,src.name);
	});
...
void ipl::Scene::Finalize(const std::shared_ptr<VFilePtrInternal> &f,const FinalizeInfo &info,const std::function<void(LoadStage,float)> &fStageProgress,const std::function<void(void)> &fOnComplete,const std::function<void(IPLerror)> &errorHandler)
{
	...
	m_finalizeData->AddThread([this,f,fOnComplete,errorHandler,fStageProgress,info]() {
		...
		if(f != nullptr)
		{
			auto err = Load(f,[](IPLfloat32 progress) {
				loadStageProgressCallback(LoadStage::LoadFromDisk,progress);
			});
		...
		else
		{
			...
			IPLhandle scene;
			auto err = iplCreateScene(
				iplContext,nullptr,m_iplSimSettings,m_materials.size(),m_materials.data(),
			...
			m_iplScene = std::shared_ptr<void>(scene,[](IPLhandle scene) {
		...
		if(bBakeData == true)
		{
			...
			if(m_probeBoxes.empty() == false)
			{
				...
				for(auto &probeBox : m_probeBoxes)
				{
					...
					std::array<float,16> m = {
						iplScale.x,0.f,0.f,0.f,
						0.f,iplScale.y,0.f,0.f,
						0.f,0.f,iplScale.z,0.f,
						iplCenter.x,iplCenter.y,iplCenter.z,1.f
					};
					IPLhandle probeHandle = nullptr;
					auto err = iplCreateProbeBox(iplContext,m_iplScene.get(),m.data(),probePlacementParams,[](float progress) {
						loadStageProgressCallback(LoadStage::GeneratingProbes,progress);
					},&probeHandle);
					if(err == IPLerror::IPL_STATUS_SUCCESS)
			...
			if(bHasSphere == true)
			{
				sgProbeBox = [this]() mutable {
					m_iplProbeBoxes.clear();
				};
				...
				{
					m_iplProbeBatches = {
						std::shared_ptr<void>(probeBatch,[](IPLhandle probeBatch) {
							iplDestroyProbeBatch(&probeBatch);
						})
					};
					sgProbeBatch = [this]() mutable {
						m_iplProbeBatches.clear();
					};
					...
					if(err == IPLerror::IPL_STATUS_SUCCESS)
					{
						m_iplProbeManager = std::shared_ptr<void>(probeManager,[](IPLhandle probeManager) {
							iplDestroyProbeManager(&probeManager);
						});
						sgProbeManager = [this]() mutable {
							m_iplProbeManager = nullptr;
						};
			...
			auto tBakeReverb = m_finalizeData->AddThread([this,errorHandler,bakingSettings]() {
				// Reverb
				iplBakeReverb(m_iplEnv.get(),m_iplProbeBoxes.front().get(),bakingSettings,[](float progress) {
					loadStageProgressCallback(LoadStage::BakingReverb,progress);
				});
			});
			...
			auto tBakePropagation = m_finalizeData->AddThread([this,bakingSettings]() {
				...
				for(auto &src : m_propagationSoundSources)
				{
					...
					iplBakePropagation(m_iplEnv.get(),m_iplProbeBoxes.front().get(),sourceInfluence,identifier,bakingSettings,[](float progress) {
						loadStageProgressCallback(LoadStage::BakingPropagation,progress);
					});
...
void ipl::Scene::RegisterPropagationSoundSource(const std::string &name,const Vector3 &origin,float radius)
{
	...
	auto it = std::find_if(m_propagationSoundSources.begin(),m_propagationSoundSources.end(),[&name](const PropagationSoundSource &src) {
		return ustring::compare(src.name,name,false);
	});
	if(it == m_propagationSoundSources.end())
	{
...
IPLDirectSoundPath ipl::Scene::GetDirectSoundPath(const Vector3 &listenerPos,const Vector3 &listenerDir,const Vector3 &listenerUp,const Vector3 &srcPos,float srcRadius) const
{
	...
	return iplGetDirectSoundPath(
		SceneState::get()->environmentalRenderer(),al::to_custom_vector<IPLVector3>(al::to_audio_position(listenerPos)),al::to_custom_vector<IPLVector3>(al::to_audio_direction(listenerDir)),
		al::to_custom_vector<IPLVector3>(al::to_audio_direction(listenerUp)),src,al::to_audio_distance(srcRadius),
		IPLDirectOcclusionMode::IPL_DIRECTOCCLUSION_NOTRANSMISSION,IPLDirectOcclusionMethod::IPL_DIRECTOCCLUSION_VOLUMETRIC
	);
}
