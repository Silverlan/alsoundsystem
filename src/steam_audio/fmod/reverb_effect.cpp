...
FMOD_DSP_PARAMETER_DESC gReverbParamSourcePosition = { FMOD_DSP_PARAMETER_TYPE_DATA, "SourcePos", "", "Position of the source." };
FMOD_DSP_PARAMETER_DESC gReverbParamName = { FMOD_DSP_PARAMETER_TYPE_DATA, "Name", "", "Unique identifier for the source." };

FMOD_DSP_PARAMETER_DESC* gReverbEffectParams[] =
{
    &gReverbParamIndirectBinaural,
    &gReverbParamSimulationType,
    &gReverbParamSourcePosition,
    &gReverbParamName
};
...
void initReverbParamDescs()
{
    ...
    gReverbParamSourcePosition.datadesc = { FMOD_DSP_PARAMETER_DATA_TYPE_3DATTRIBUTES };
    gReverbParamName.datadesc = { FMOD_DSP_PARAMETER_DATA_TYPE_USER };
}
...
public:
    ...
    ReverbEffectState() :
		...
		identifier{0,IPLBakedDataType::IPL_BAKEDDATATYPE_REVERB}
    {}
                    ...
                    const IPLAudioFormat outputFormat)
    {
        ...
        {
			ipl::check_result(gApi.iplCreateConvolutionEffect(mSceneState->environmentalRenderer(), identifier, type,
                mInputFormat, mIndirectEffectOutputBuffer.format, &mConvolutionEffect));
            if (!mConvolutionEffect)
            {
                return false;
            }
        }
...
FMOD_RESULT F_CALL setReverbData(FMOD_DSP_STATE* state, 
                                      int index, 
                                      void* data, 
                                      unsigned int length)
{
    ...
    if (static_cast<ReverbEffectParams>(index) == SA_REVERB_PARAM_SOURCEPOSITION)
    {
        memcpy(&params->sourcePosition, data, length);
        return FMOD_OK;
    }
    else if (static_cast<ReverbEffectParams>(index) == SA_REVERB_PARAM_NAME)
    {
